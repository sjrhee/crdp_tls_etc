...
## 1. tls 연결
- 처음 연결 시 tls 협상 과정에서 성능 저하
- 세션이 반복 재사용되는 경우 캐시되어 영향이 줄어듬

## 2. JWT 사용
- 서명 검증 및 클레임 추출 시간 필요
- 데이터 전송량 증가에 따른 네트워크 지연

## 3. 접근 정책
- 사용자 처리에 추가 지연 발생
- 불허 사용자에 대한 암호문 또는 오류 처리가 가장 빠름
- 마스킹 처리 시 추가 지연 발생

## 4. 암호화 알고리즘
- AES/ECB/PKCS5Padding, AES/CBC/PKCS5Padding, FPE/AES, FPE/FF3-1, FPE/FF3, FPE/FF1v2 순
- NoPadding은 현실적으로 길이를 맞출 수 없어 제외
- FPE는 AES에 비하여 많은 추가 지연 발생
